...
# =============================================================================
# Phase 11: OpenClaw + Model Load Optimizer
# =============================================================================
echo "=== Phase 11: OpenClaw + Model Load Optimizer ==="
...
if [ "$OLLAMA_INSTALLED" = "1" ] && [ -n "$OLLAMA_MODEL" ]; then
    ...
    if [ -f "$PLUGIN_DIR/package.json" ]; then
        ...
        cd "$PLUGIN_DIR" && npm install --silent 2>/dev/null && npm run build 2>/dev/null
        echo "  Plugin built."
    fi
    ...
    if [ "$OLLAMA_MODEL" = "qwen2.5-coder:7b" ] || [ "$OLLAMA_MODEL" = "qwen2.5-coder:1.5b" ]; then
    ...
    if [ -n "$SIDECAR_MODEL" ]; then
        ...
        SIDECAR_CONFIG="\"ollama/$SIDECAR_MODEL\": { \"alias\": \"sidecar\" },"
    fi
...
OCEOF
    ...
    echo "  Skipping (no Ollama models installed)"
fi
